...
#define TEST_BRANCH_ON_QUEUE(testname, typeenum, typename, type, op, opname)                       \
    for (size_t cmp = 1; cmp <= NUM_COMPARISON_OPERATORS; cmp++) {                                 \
        ...
        ishmemx_##typename##_##testname##_on_queue(((type *) src) + ((cmp - 1) * nelems), nelems,  \
                                                   NULL, static_cast<int>(cmp), cmp_value, q);     \
    }
...
GEN_FNS(wait_until_all, NOP, nop)

class wait_until_all_tester : public ishmem_tester {
  public:
    wait_until_all_tester(int argc, char *argv[]) : ishmem_tester(argc, argv, true) {}
...
size_t wait_until_all_tester::create_source_pattern(ishmemi_type_t t, ishmemi_op_t op,
...
size_t wait_until_all_tester::create_check_pattern(ishmemi_type_t t, ishmemi_op_t op,
                                                   testmode_t mode, size_t nelems)
{
    return 0;
}
...
{
    class wait_until_all_tester t(argc, argv);
    ...
    GEN_TABLES(wait_until_all, NOP, nop)
