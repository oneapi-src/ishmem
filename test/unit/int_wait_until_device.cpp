...
int main(int argc, char **argv)
{
    ...
    if (npes % 2) {
        std::cerr << "[WARN] int_wait_until_device must be run with an even "
                     "number of PEs"
                  << std::endl;
        ishmem_finalize();
        return exit_code;
    ...
    if (my_pe % 2) {
        auto e1 = q.submit([&](sycl::handler &h) {
            h.parallel_for(sycl::nd_range<1>{NUM_TESTS, NUM_TESTS}, [=](sycl::nd_item<1> it) {
                ...
                if (id == 0) ishmem_int_wait_until(&source[0], ISHMEM_CMP_EQ, 0);
                else if (id == 1) ishmem_int_wait_until(&source[1], ISHMEM_CMP_NE, 1);
                else if (id == 2) ishmem_int_wait_until(&source[2], ISHMEM_CMP_GT, 2);
                else if (id == 3) ishmem_int_wait_until(&source[3], ISHMEM_CMP_GE, 3);
                else if (id == 4) ishmem_int_wait_until(&source[4], ISHMEM_CMP_LT, 1);
                else if (id == 5) ishmem_int_wait_until(&source[5], ISHMEM_CMP_LE, 0);
            });
        });
        e1.wait_and_throw();
    } else {
