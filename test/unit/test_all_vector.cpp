...
#define TEST_BRANCH_SINGLE(testname, typeenum, typename, type, op, opname)                         \
    ...
    ret = static_cast<uint64_t>(ishmem_##typename##_##testname(                                    \
        (type *) src, nelems, NULL, static_cast<int>(cmp), cmp_values));                           \
                                                                                                   \
...
#define TEST_BRANCH_WORK_GROUP(testname, typeenum, typename, type, op, opname)                     \
    ...
    ret = static_cast<uint64_t>(ishmemx_##typename##_##testname##_work_group(                      \
        (type *) src, nelems, NULL, static_cast<int>(cmp), cmp_values, grp));                      \
                                                                                                   \
    if (grp.leader()) memcpy((uint64_t *) dest + (cmp - 1), &ret, sizeof(uint64_t));

GEN_FNS(test_all_vector, NOP, nop)

class test_all_vector_tester : public ishmem_tester {
  public:
    test_all_vector_tester(int argc, char *argv[]) : ishmem_tester(argc, argv) {}
...
size_t test_all_vector_tester::create_source_pattern(ishmemi_type_t t, ishmemi_op_t op,
                                                     testmode_t mode, size_t nelems)
{
    size_t test_size = nelems * typesize(t);
    for (size_t i = 0; i < nelems; i++) {
        if (typesize(t) == sizeof(uint64_t)) {
            ((uint64_t *) aligned_source)[i] = (i % 10) + 5;
        } else if (typesize(t) == sizeof(uint32_t)) {
            ((uint32_t *) aligned_source)[i] = (i % 10) + 5;
        } else if (typesize(t) == sizeof(uint16_t)) {
            ((uint16_t *) aligned_source)[i] = (i % 10) + 5;
        } else if (typesize(t) == sizeof(uint8_t)) {
            ((uint8_t *) aligned_source)[i] = (i % 10) + 5;
        }
    }
...
size_t test_all_vector_tester::create_check_pattern(ishmemi_type_t t, ishmemi_op_t op,
                                                    testmode_t mode, size_t nelems)
{
...
int main(int argc, char **argv)
{
    ...
    GEN_TABLES(test_all_vector, NOP, nop)
