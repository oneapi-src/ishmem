...
GEN_ARITHMETIC_FNS(reduce, PROD_REDUCE, prod)

unsigned long tprod(void *a, void *b, ishmemi_type_t t, size_t index)
{
    switch (t) {
        case FLOAT: {
            float my_prod = ((float *) a)[index] * ((float *) b)[index];
            return (unsigned long) std::bitset<sizeof(float) * BITS_PER_BYTE>(
                       static_cast<unsigned long long>(
                           *reinterpret_cast<unsigned long *>(&my_prod)))
                .to_ulong();
        }
        case DOUBLE: {
            double my_prod = ((double *) a)[index] * ((double *) b)[index];
...
class reduce_prod_tester : public ishmem_tester {
  public:
    reduce_prod_tester(int argc, char *argv[]) : ishmem_tester(argc, argv, true) {}
...
size_t reduce_prod_tester::create_source_pattern(ishmemi_type_t t, ishmemi_op_t op, testmode_t mode,
                                                 size_t nelems)
{
...
size_t reduce_prod_tester::create_check_pattern(ishmemi_type_t t, ishmemi_op_t op, testmode_t mode,
                                                size_t nelems)
{
    ...
    if (t == FLOAT) {
        ...
        for (size_t idx = 0; idx < ((test_size / sizeof(long)) + 1); idx += 1) {
            ...
            for (size_t j = 0; j < sizeof(long) / typesize(t); j++) {
                ...
                long cur_prod = static_cast<long>(((idx % 2) << 48) + (((idx + 1) % 2) << 40) +
                                                  (((idx + 2) % 2) << 32) + ((idx + 3) % 2));
                for (int i = 1; i < n_pes; i++) {
                    ...
                    cur_prod = static_cast<long>(
                        tprod(&cur_prod, &cmp, t, ((sizeof(long) / typesize(t)) - (j + 1))));
                    cur_prod <<=
                        ((sizeof(long) / typesize(t)) - (j + 1)) * ((typesize(t)) * BITS_PER_BYTE);
                }
                expected |= (cur_prod & mask);
            }
...
{
    class reduce_prod_tester t(argc, argv);
    ...
    GEN_ARITHMETIC_TABLES(reduce, PROD_REDUCE, prod)

    if (!t.test_types_set) t.add_test_type_list(arithmetic_reduction_types);
    errors += t.run_aligned_tests(PROD_REDUCE);
    errors += t.run_offset_tests(PROD_REDUCE);
