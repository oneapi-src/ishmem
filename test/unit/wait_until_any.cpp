...
#define ATOMIC_SET_IVARS_WAIT_UNTIL_ANY(typename, type)                                            \
    do {                                                                                           \
        for (size_t cmp = 1; cmp <= NUM_COMPARISON_OPERATORS; cmp++) {                             \
            ...
            ishmem_##typename##_atomic_set(((type *) src) + ((cmp - 1) * nelems) + (nelems / 2),   \
                                           val, (ishmem_my_pe() + 1) % ishmem_n_pes());            \
            ishmem_quiet();                                                                        \
        }                                                                                          \
    } while (0);
...
#define TEST_BRANCH_WORK_GROUP(testname, typeenum, typename, type, op, opname)                     \
    ...
    for (size_t cmp = 1; cmp <= NUM_COMPARISON_OPERATORS; cmp++) {                                 \
        ...
        ret = ishmemx_##                                                                           \
            typename##_##testname##_work_group(((type *) src) + ((cmp - 1) * nelems), nelems,      \
                                               NULL, static_cast<int>(cmp), cmp_value, grp);       \
        if (grp.leader()) memcpy((size_t *) dest + (cmp - 1), &ret, sizeof(size_t));               \
    }

GEN_FNS(wait_until_any, NOP, nop)

class wait_until_any_tester : public ishmem_tester {
  public:
    wait_until_any_tester(int argc, char *argv[]) : ishmem_tester(argc, argv, true) {}
...
size_t wait_until_any_tester::create_source_pattern(ishmemi_type_t t, ishmemi_op_t op,
...
size_t wait_until_any_tester::create_check_pattern(ishmemi_type_t t, ishmemi_op_t op,
                                                   testmode_t mode, size_t nelems)
{
...
{
    class wait_until_any_tester t(argc, argv);
    ...
    GEN_TABLES(wait_until_any, NOP, nop)
