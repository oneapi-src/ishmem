...
unsigned long tmin(void *a, void *b, ishmemi_type_t t, size_t index)
{
    switch (t) {
        case FLOAT: {
            float my_min = std::min(((float *) a)[index], ((float *) b)[index]);
            return (unsigned long) std::bitset<sizeof(float) * BITS_PER_BYTE>(
                       static_cast<unsigned long long>(*reinterpret_cast<unsigned long *>(&my_min)))
                .to_ulong();
        }
        case DOUBLE: {
            double my_min = std::min(((double *) a)[index], ((double *) b)[index]);
...
class reduce_min_tester : public ishmem_tester {
  public:
    reduce_min_tester(int argc, char *argv[]) : ishmem_tester(argc, argv, true) {}
...
size_t reduce_min_tester::create_source_pattern(ishmemi_type_t t, ishmemi_op_t op, testmode_t mode,
                                                size_t nelems)
{
...
size_t reduce_min_tester::create_check_pattern(ishmemi_type_t t, ishmemi_op_t op, testmode_t mode,
                                               size_t nelems)
{
...
{
    class reduce_min_tester t(argc, argv);
