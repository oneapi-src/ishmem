        ...
        auto e_run = q.submit([&](sycl::handler &h) {                                              \
            h.single_task([=]() {                                                                  \
                for (int i = 0; i < npes; i++)                                                     \
                    ishmem_##TYPENAME##_atomic_fetch_inc_nbi(&local[i], remote, i);                \
                ishmem_barrier_all();                                                              \
            });                                                                                    \
        });                                                                                        \
...
#define TEST_SHMEM_FETCH_INC_PARALLEL_FOR(TYPE, TYPENAME)                                          \
    do {                                                                                           \
        ...
        q.single_task([=]() { ishmem_barrier_all(); }).wait_and_throw();                           \
        auto e_run = q.parallel_for(sycl::nd_range<1>{N, N}, [=](sycl::nd_item<1> idx) {           \
            size_t i = idx.get_global_id(0);                                                       \
            auto grp = idx.get_group();                                                            \
            for (size_t j = 0; j < npes; j++)                                                      \
                ishmem_##TYPENAME##_atomic_fetch_inc_nbi(&val[j + i * (size_t) npes], &remote[i],  \
                                                         static_cast<int>(j));                     \
