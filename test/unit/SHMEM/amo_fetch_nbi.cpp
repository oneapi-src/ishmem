...
#define TEST_SHMEM_FETCH_SINGLE_TASK(TYPE, TYPENAME)                                               \
    do {                                                                                           \
        ...
        auto e_run = q.submit([&](sycl::handler &h) {                                              \
            h.single_task([=]() {                                                                  \
                ishmem_##TYPENAME##_atomic_fetch_nbi(&local[0], remote, (mype + 1) % npes);        \
                ishmem_barrier_all();                                                              \
            });                                                                                    \
        });                                                                                        \
...
#define TEST_SHMEM_FETCH_PARALLEL_FOR(TYPE, TYPENAME)                                              \
    do {                                                                                           \
        ...
        CHECK_ALLOC(remote);                                                                       \
        TYPE *val = sycl::malloc_host<TYPE>(N, q);                                                 \
        CHECK_ALLOC(val);                                                                          \
        int *errors = sycl::malloc_host<int>(1, q);                                                \
        CHECK_ALLOC(errors);                                                                       \
        ishmem_barrier_all();                                                                      \
        ...
        auto e_run = q.parallel_for(sycl::nd_range<1>{N, N}, [=](sycl::nd_item<1> idx) {           \
            ...
            ishmem_##TYPENAME##_atomic_fetch_nbi(&val[i], &remote[i], (mype + 1) % npes);          \
