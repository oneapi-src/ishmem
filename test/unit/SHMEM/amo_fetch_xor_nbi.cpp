...
#define TEST_SHMEM_FETCH_XOR_SINGLE_TASK(TYPE, TYPENAME)                                           \
    do {                                                                                           \
        ...
        auto e_run = q.submit([&](sycl::handler &h) {                                              \
            h.single_task([=]() {                                                                  \
                for (int i = 0; i < npes; i++)                                                     \
                    ishmem_##TYPENAME##_atomic_fetch_xor_nbi(&local[i], remote,                    \
                                                             (TYPE) (1LLU << mype), i);            \
                ishmem_barrier_all();                                                              \
            });                                                                                    \
        });                                                                                        \
...
#define TEST_SHMEM_FETCH_XOR_PARALLEL_FOR(TYPE, TYPENAME)                                          \
    do {                                                                                           \
        ...
        auto e_run = q.parallel_for(sycl::nd_range<1>{N, N}, [=](sycl::nd_item<1> idx) {           \
            ...
            for (size_t j = 0; j < npes; j++)                                                      \
                ishmem_##TYPENAME##_atomic_fetch_xor_nbi(&val[i * (size_t) npes + j], &remote[i],  \
                                                         (TYPE) (1LLU << mype),                    \
                                                         static_cast<int>(j));                     \
...
int main(int argc, char *argv[])
{
    ...
    TEST_SHMEM_FETCH_XOR_SINGLE_TASK(unsigned int, uint);
    TEST_SHMEM_FETCH_XOR_SINGLE_TASK(unsigned long, ulong);
    TEST_SHMEM_FETCH_XOR_SINGLE_TASK(unsigned long long, ulonglong);
    TEST_SHMEM_FETCH_XOR_SINGLE_TASK(int32_t, int32);
    TEST_SHMEM_FETCH_XOR_SINGLE_TASK(int64_t, int64);
    TEST_SHMEM_FETCH_XOR_SINGLE_TASK(uint32_t, uint32);
    TEST_SHMEM_FETCH_XOR_SINGLE_TASK(uint64_t, uint64);

    TEST_SHMEM_FETCH_XOR_PARALLEL_FOR(unsigned int, uint);
    TEST_SHMEM_FETCH_XOR_PARALLEL_FOR(unsigned long, ulong);
    TEST_SHMEM_FETCH_XOR_PARALLEL_FOR(unsigned long long, ulonglong);
    TEST_SHMEM_FETCH_XOR_PARALLEL_FOR(int32_t, int32);
    TEST_SHMEM_FETCH_XOR_PARALLEL_FOR(int64_t, int64);
    TEST_SHMEM_FETCH_XOR_PARALLEL_FOR(uint32_t, uint32);
    TEST_SHMEM_FETCH_XOR_PARALLEL_FOR(uint64_t, uint64);
