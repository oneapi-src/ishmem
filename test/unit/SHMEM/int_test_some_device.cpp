...
int main(int argc, char **argv)
{
    ...
    *errors = 0;
    auto e1 = q.submit([&](sycl::handler &h) {
        h.single_task([=]() {
            ...
            ret[6] = ishmem_int_test_some(source, ARRAY_SIZE, indices[6], NULL, ISHMEM_CMP_EQ,
                                          ARRAY_SIZE);
            ret[7] = ishmem_int_test_some(source, ARRAY_SIZE, indices[7], NULL, ISHMEM_CMP_GT,
                                          ARRAY_SIZE);
            ret[8] = ishmem_int_test_some(source, ARRAY_SIZE, indices[8], NULL, ISHMEM_CMP_GE,
                                          ARRAY_SIZE);
            ret[9] = ishmem_int_test_some(source, ARRAY_SIZE, indices[9], NULL, ISHMEM_CMP_LT, -1);
            ret[10] =
                ishmem_int_test_some(source, ARRAY_SIZE, indices[10], NULL, ISHMEM_CMP_LE, -1);
            ret[11] =
                ishmem_int_test_some(source, ARRAY_SIZE, indices[11], status, ISHMEM_CMP_NE, 1);
        });
    });
    e1.wait_and_throw();
    ...
    auto e2 = q.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::nd_range<1>{NUM_WORK_ITEMS, NUM_WORK_ITEMS}, [=](sycl::nd_item<1> it) {
            ...
            // At least one entry in source satisfies the "cmp" condition
            ret[0] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[0], NULL,
                                                      ISHMEM_CMP_EQ, 0, grp);
            ret[1] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[1], NULL,
                                                      ISHMEM_CMP_NE, 1, grp);
            ret[2] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[2], NULL,
                                                      ISHMEM_CMP_GT, 2, grp);
            ret[3] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[3], NULL,
                                                      ISHMEM_CMP_GE, 3, grp);
            ret[4] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[4], NULL,
                                                      ISHMEM_CMP_LT, 1, grp);
            ret[5] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[5], NULL,
                                                      ISHMEM_CMP_LE, 0, grp);

            // No entries satisfy "cmp" condition
            ret[6] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[6], NULL,
                                                      ISHMEM_CMP_EQ, ARRAY_SIZE, grp);
            ret[7] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[7], NULL,
                                                      ISHMEM_CMP_GT, ARRAY_SIZE, grp);
            ret[8] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[8], NULL,
                                                      ISHMEM_CMP_GE, ARRAY_SIZE, grp);
            ret[9] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[9], NULL,
                                                      ISHMEM_CMP_LT, -1, grp);
            ret[10] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[10], NULL,
                                                       ISHMEM_CMP_LE, -1, grp);
            ret[11] = ishmemx_int_test_some_work_group(source, ARRAY_SIZE, indices[12], status,
                                                       ISHMEM_CMP_NE, 1, grp);
        });
    });
    e2.wait_and_throw();
    ...
    *errors = 0;
    ...
    if (*errors > 0) {
        std::cerr << "[ERROR] work_group validation check(s) failed: " << *errors << std::endl;
        for (size_t i = 0; i < NUM_TESTS; i += 1) {
            if (ret[i] != ret_check[i]) {
                std::cerr << "[" << my_pe << "]: ret[" << i << "] = " << ret[i] << ", expected "
                          << ret_check[i] << std::endl;
