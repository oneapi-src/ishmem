    ...
    auto e1 = q.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::nd_range<1>(sycl::range<1>(x_size), sycl::range<1>(x_size)),
                       [=](sycl::nd_item<1> it) {
                           ...
                           ishmemx_int_iget_work_group(dest_group_1d, source, dst, sst,
    ...
    auto e2 = q.submit([&](sycl::handler &h) {
        h.parallel_for(
            ...
            [=](sycl::nd_item<2> it) {
                ...
                auto grp = it.get_group();
                ishmemx_int_iget_work_group(dest_group_2d, source, dst, sst, elems_to_copy,
    ...
    auto e3 = q.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::nd_range<3>(sycl::range<3>(x_size, y_size, z_size),
                       ...
                       [=](sycl::nd_item<3> it) {
                           ...
                           auto grp = it.get_group();
                           ishmemx_int_iget_work_group(dest_group_3d, source, dst, sst,
                                                       elems_to_copy, (my_dev_pe + 1) % my_dev_npes,
                                                       grp);
                       });
    });
    ...
    auto e4 = q.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::nd_range<3>(sycl::range<3>(x_size, y_size, z_size),
                       ...
                       [=](sycl::nd_item<3> it) {
                           ...
                           ishmemx_int_iget_work_group(dest_sub_group + (sub_group_start_idx * dst),
