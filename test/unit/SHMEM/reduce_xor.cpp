...
#define ISHMEM_TYPE_BRANCH(enumname, name, type)                                                   \
    res = ishmem_##name##_xor_reduce((type *) dest, (type *) src, nelems);                         \
    break;
...
#define ISHMEM_TYPE_BRANCH(enumname, name, type)                                                   \
    res = ishmemx_##name##_xor_reduce_work_group((type *) dest, (type *) src, nelems, grp);        \
    break;
...
class reduce_xor_tester : public ishmem_tester {
  public:
    reduce_xor_tester(int argc, char *argv[]) : ishmem_tester(argc, argv) {}
