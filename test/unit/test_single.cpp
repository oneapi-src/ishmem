...
#define TEST_BRANCH_SINGLE(testname, typeenum, typename, type, op, opname)                         \
    ...
    for (size_t cmp = 1; cmp <= NUM_COMPARISON_OPERATORS; cmp++) {                                 \
        ...
        ret = static_cast<uint64_t>(                                                               \
            ishmem_##typename##_##testname((type *) src, static_cast<int>(cmp), cmp_value));       \
                                                                                                   \
...
#define TEST_BRANCH_WORK_GROUP(testname, typeenum, typename, type, op, opname)                     \
    ...
    for (size_t cmp = 1; cmp <= NUM_COMPARISON_OPERATORS; cmp++) {                                 \
        ...
        ret = static_cast<uint64_t>(ishmemx_##typename##_##testname##_work_group(                  \
            (type *) src, static_cast<int>(cmp), cmp_value, grp));                                 \
                                                                                                   \
        if (grp.leader()) memcpy((uint64_t *) dest + (cmp - 1), &ret, sizeof(uint64_t));           \
    }

GEN_FNS(test, NOP, nop)

class test_tester : public ishmem_tester {
  public:
    test_tester(int argc, char *argv[]) : ishmem_tester(argc, argv) {}
...
size_t test_tester::create_source_pattern(ishmemi_type_t t, ishmemi_op_t op, testmode_t mode,
                                          size_t nelems)
{
...
size_t test_tester::create_check_pattern(ishmemi_type_t t, ishmemi_op_t op, testmode_t mode,
                                         size_t nelems)
{
