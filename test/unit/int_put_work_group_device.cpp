...
int main(int argc, char **argv)
{
    ...
    auto e_init = q.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::nd_range<1>{array_size, array_size}, [=](sycl::nd_item<1> idx) {
            ...
            source[i] =
                0x4000000 + (my_pe << 24) + (((my_pe + 1) % npes) << 20) + static_cast<int>(i);
            dest_group_1d[i] = 0;
            dest_group_2d[i] = 0;
            dest_group_3d[i] = 0;
            dest_sub_group[i] = 0;
    ...
    /* Perform put work group operations */
    auto e1 = q.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::nd_range<1>(sycl::range<1>(x_size), sycl::range<1>(x_size)),
                       [=](sycl::nd_item<1> it) {
                           ...
                           ishmemx_int_put_work_group(dest_group_1d, source, array_size,
    ...
    auto e2 = q.submit([&](sycl::handler &h) {
        h.parallel_for(
            ...
            [=](sycl::nd_item<2> it) {
                ...
                auto grp = it.get_group();
                ishmemx_putmem_work_group(dest_group_2d, source, array_size * sizeof(int),
                                          (my_dev_pe + 1) % my_dev_npes, grp);
            });
    });
    auto e3 = q.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::nd_range<3>(sycl::range<3>(x_size, y_size, z_size),
                       ...
                       [=](sycl::nd_item<3> it) {
                           ...
                           auto grp = it.get_group();
                           ishmemx_int_put_nbi_work_group(dest_group_3d, source, array_size,
                                                          (my_dev_pe + 1) % my_dev_npes, grp);
                       });
    });
    auto e4 = q.submit([&](sycl::handler &h) {
        h.parallel_for(sycl::nd_range<3>(sycl::range<3>(x_size, y_size, z_size),
                       ...
                       [=](sycl::nd_item<3> it) {
                           ...
                           ishmemx_putmem_nbi_work_group(dest_sub_group + sub_group_start_idx,
    ...
    auto e_verify = q.submit([&](sycl::handler &h) {
        h.single_task([=]() {
            ...
            for (int i = 0; i < array_size; ++i) {
                int expected_value = 0x4000000 + (source_pe << 24) + (my_pe << 20) + (i);
