    ...
    switch (cmp) {                                                                                 \
        case ISHMEM_CMP_EQ:                                                                        \
            for (size_t i = 0; (i == 0) || (i < nelems / 2); i++) {                                \
                ((type *) _cmp_values)[i] = 0;                                                     \
            }                                                                                      \
            if (nelems > 1) {                                                                      \
                for (size_t i = nelems / 2; i < nelems; i++) {                                     \
                    ((type *) _cmp_values)[i] = (i % 5) + 11;                                      \
                }                                                                                  \
            }                                                                                      \
            break;                                                                                 \
        case ISHMEM_CMP_NE:                                                                        \
            for (size_t i = 0; (i == 0) || (i < nelems / 2); i++) {                                \
                ((type *) _cmp_values)[i] = (i % 5) + 1;                                           \
            }                                                                                      \
            if (nelems > 1) {                                                                      \
                for (size_t i = nelems / 2; i < nelems; i++) {                                     \
        ...
        case ISHMEM_CMP_LT:                                                                        \
            ...
            if (nelems > 1) {                                                                      \
                for (size_t i = nelems / 2; i < nelems; i++) {                                     \
                    ((type *) _cmp_values)[i] = (i % 5) + 12;                                      \
                }                                                                                  \
            }                                                                                      \
            break;                                                                                 \
        case ISHMEM_CMP_LE:                                                                        \
            for (size_t i = 0; (i == 0) || (i < nelems / 2); i++) {                                \
                ((type *) _cmp_values)[i] = (i % 5);                                               \
            }                                                                                      \
            if (nelems > 1) {                                                                      \
                for (size_t i = nelems / 2; i < nelems; i++) {                                     \
                    ((type *) _cmp_values)[i] = (i % 5) + 11;                                      \
...
GEN_FNS(test_some_vector, NOP, nop)

class test_some_vector_tester : public ishmem_tester {
  public:
    test_some_vector_tester(int argc, char *argv[]) : ishmem_tester(argc, argv) {}
...
size_t test_some_vector_tester::create_source_pattern(ishmemi_type_t t, ishmemi_op_t op,
                                                      testmode_t mode, size_t nelems)
{
    size_t test_size = nelems * typesize(t);
    for (size_t i = 0; i < nelems; i++) {
        if (typesize(t) == sizeof(uint64_t)) {
            ((uint64_t *) aligned_source)[i] =
                (((i >= nelems / 2) && (nelems > 1))) ? (i % 5) + 11 : (i % 5) + 1;
        } else if (typesize(t) == sizeof(uint32_t)) {
            ((uint32_t *) aligned_source)[i] =
                (((i >= nelems / 2) && (nelems > 1))) ? (i % 5) + 11 : (i % 5) + 1;
        } else if (typesize(t) == sizeof(uint16_t)) {
            ((uint16_t *) aligned_source)[i] =
                (((i >= nelems / 2) && (nelems > 1))) ? (i % 5) + 11 : (i % 5) + 1;
        } else if (typesize(t) == sizeof(uint8_t)) {
            ((uint8_t *) aligned_source)[i] =
                (((i >= nelems / 2) && (nelems > 1))) ? (i % 5) + 11 : (i % 5) + 1;
        }
    }
...
size_t test_some_vector_tester::create_check_pattern(ishmemi_type_t t, ishmemi_op_t op,
                                                     testmode_t mode, size_t nelems)
{
...
{
    class test_some_vector_tester t(argc, argv);
    t.max_nelems = 512;
    ...
    GEN_TABLES(test_some_vector, NOP, nop)
