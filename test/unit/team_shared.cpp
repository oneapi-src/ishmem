 ...
 * PEs in ISHMEM_TEAM_SHARED. Then each PE nominiates a leader PE from from
 * ISHMEM_TEAM_SHARED to accumulate the buffer across teams in a hierarchical
 * manner.  This test will fail if:
 *     1) It is executed with fewer than 2 PEs.
 *     1) Instances of ISHMEM_TEAM_SHARED have differing sizes (e.g. on heterogeneous platforms).
...
int main(void)
{
    int my_pe, npes, team_shared_n_pes;
    ...
    team_shared_n_pes = ishmem_team_n_pes(ISHMEM_TEAM_SHARED);

    int *peers = (int *) malloc(static_cast<size_t>(team_shared_n_pes) * sizeof(int));
    int num_peers = 0;

    sycl::queue q;

    /* Print the team members on ISHMEM_TEAM_SHARED. */
    /* Synchronize each iteration for clean output. */
    for (int pe = 0; pe < npes; pe++) {
        if (pe == my_pe) {
            std::cout << "[PE: " << my_pe << "] ISHMEM_TEAM_SHARED peers: { ";
            for (int i = 0; i < npes; i++) {
                if (ishmem_team_translate_pe(ISHMEM_TEAM_WORLD, i, ISHMEM_TEAM_SHARED) != -1) {
                    peers[num_peers++] = i;
                    std::cout << i << " ";
                }
            }
    ...
    if (num_peers != team_shared_n_pes) {
        std::cerr << "Inconsistent number of peers in ISHMEM_TEAM_SHARED" << std::endl;
        ishmem_finalize();
        return 1;
    }
    ...
    *leader_stride_host = 0;
    ...
    e_init = q.submit([&](sycl::handler &h) {
        h.single_task([=]() {
            /* Local sum reduce across ISHMEM_TEAM_SHARED: */
            ishmem_int_sum_reduce(ISHMEM_TEAM_SHARED, target, source, array_size);

            /* Check the reduction result: */
            for (int i = 0; i < array_size; i++) {
                int cur_sum = 0;
                for (int j = 0; j < team_shared_n_pes; j++) {
                    int world_pe =
                        ishmem_team_translate_pe(ISHMEM_TEAM_SHARED, j, ISHMEM_TEAM_WORLD);
            ...
            *leader_pe = my_pe;
            ...
            ishmem_team_sync(ISHMEM_TEAM_SHARED);

            /* The leader PE is the one with the smallest world PE ID */
            ishmem_int_min_reduce(ISHMEM_TEAM_SHARED, leader_pe, leader_pe, 1);

            for (int i = 0; i < npes; i++)
                leader_pes[i] = -1;

            /* Verify that all ISHMEM_TEAM_SHARED team instances have the same number of PEs */
            team_size[0] = team_shared_n_pes;
            ...
            for (int i = 0; i < npes; i++)
                if (team_sizes_collect[i] != team_shared_n_pes) *errors += 1;
            ...
            assert(npes > 1 && leader_pes[team_shared_n_pes] != -1);
            int curr_stride = leader_pes[team_shared_n_pes] - leader_pes[0];
            for (int i = team_shared_n_pes; i + team_shared_n_pes < npes; i += team_shared_n_pes) {
                int stride = 0;
                if (leader_pes[i + team_shared_n_pes] != -1) {
                    stride = leader_pes[i + team_shared_n_pes] - leader_pes[i];
                } else {
                    break;
                }
    ...
    /* Verify and print ISHMEM_TEAM_SHARED sum reduction result: */
    if (*errors > 0) {
        ...
        for (int i = 0; i < array_size; ++i) {
            int cur_sum = 0;
            for (int j = 0; j < team_shared_n_pes; ++j) {
                int world_pe = ishmem_team_translate_pe(ISHMEM_TEAM_SHARED, j, ISHMEM_TEAM_WORLD);
                cur_sum += ((i % (world_pe + 2)) << 16) + i;
            }
    ...
    assert(npes % team_shared_n_pes == 0);

    /* Create the leader PE team: */
    ishmem_team_t leader_team;
    ishmem_team_split_strided(ISHMEM_TEAM_WORLD, 0, *leader_stride_host, npes / team_shared_n_pes,
                              NULL, 0, &leader_team);

    /* The Leader PEs now reduce the results of the previous ISHMEM_TEAM_SHARED sum reduction: */
