...
int main(void)
{
    ...
    *errors = 0;
    ...
    int ret = ishmem_team_split_strided(ISHMEM_TEAM_WORLD, npes - 1, -1, npes, NULL, 0, &new_team);
    if (ret != 0) printf("[%d] ishmem_team_split_strided failed, got team %d\n", me, new_team);
    ...
    ret = ishmem_team_split_strided(ISHMEM_TEAM_WORLD, npes - 1, -2, npes / 2, NULL, 0, &new_team);
    if (ret != 0) printf("[%d] ishmem_team_split_strided failed, got team %d\n", me, new_team);
