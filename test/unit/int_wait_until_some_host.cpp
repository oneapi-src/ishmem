...
int main(int argc, char **argv)
{
    ...
    if (my_pe % 2) {
        do {
            ret[0] = ishmem_int_wait_until_some(source[0], ARRAY_SIZE, indices[0], NULL,
                                                ISHMEM_CMP_EQ, 0);
        } while (ret[0] != ret_check[0]);

        do {
            ret[1] = ishmem_int_wait_until_some(source[1], ARRAY_SIZE, indices[1], NULL,
                                                ISHMEM_CMP_NE, 1);
        } while (ret[1] != ret_check[1]);

        do {
            ret[2] = ishmem_int_wait_until_some(source[2], ARRAY_SIZE, indices[2], NULL,
                                                ISHMEM_CMP_GT, 2);
        } while (ret[2] != ret_check[2]);

        do {
            ret[3] = ishmem_int_wait_until_some(source[3], ARRAY_SIZE, indices[3], NULL,
                                                ISHMEM_CMP_GE, 3);
        } while (ret[3] != ret_check[3]);

        do {
            ret[4] = ishmem_int_wait_until_some(source[4], ARRAY_SIZE, indices[4], NULL,
                                                ISHMEM_CMP_LT, 1);
        } while (ret[4] != ret_check[4]);

        do {
            ret[5] = ishmem_int_wait_until_some(source[5], ARRAY_SIZE, indices[5], NULL,
                                                ISHMEM_CMP_LE, 0);
        } while (ret[5] != ret_check[5]);

        status[0] = status[1] = 1;
        do {
            ret[6] = ishmem_int_wait_until_some(source[6], ARRAY_SIZE, indices[6], status,
                                                ISHMEM_CMP_EQ, 5);
