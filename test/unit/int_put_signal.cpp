    ...
    if (my_pe == 0) {
        auto e1 = q.submit([&](sycl::handler &h) {
            h.single_task([=]() {
                ...
                int dest_pe = (my_dev_pe + 1) % my_dev_npes;

                ishmem_int_put_signal(target, source, array_size, sig_addr,
                                      static_cast<uint64_t>(dest_pe + 1), ISHMEM_SIGNAL_SET,
                                      dest_pe);
                ishmem_uint64_wait_until(sig_addr, ISHMEM_CMP_EQ,
                                         static_cast<uint64_t>(my_dev_pe + 1));
            });
        });
        e1.wait_and_throw();
    } else {
