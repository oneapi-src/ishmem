...
#define BW_TEST_FUNCTION                                                                           \
    for (int i = 0; i < iterations; i += 1) {                                                      \
        ishmem_long_alltoall(ISHMEM_TEAM_WORLD, (long *) dest, (long *) src, nelems);              \
    }

#define BW_TEST_FUNCTION_ON_QUEUE                                                                  \
    for (int i = 0; i < iterations; i += 1) {                                                      \
        ishmemx_long_alltoall_on_queue((long *) dest, (long *) src, nelems, test_return, q);       \
    }

#define BW_TEST_FUNCTION_WORK_GROUP                                                                \
    for (int i = 0; i < iterations; i += 1) {                                                      \
        ishmemx_long_alltoall_work_group(ISHMEM_TEAM_WORLD, (long *) dest, (long *) src, nelems,   \
                                         grp);                                                     \
    }
