...
``MPI_CXX_SKIP_MPICXX``
  ...
  for example MVAPICH or Intel® MPI Library.
...
# Intel® MPI Library compiler names on Windows
if(WIN32)
  ...
  # Intel® MPI Library compiler names
  ...
  # Intel® MPI Library compiler names for MSMPI
...
# stems from a proprietary MPI and won't know which compiler it's being used for. For instance, Intel® MPI Library
...
function (_MPI_check_compiler LANG QUERY_FLAG OUTPUT_VARIABLE RESULT_VARIABLE)
  ...
  # Intel® MPI Library tolerates unknown arguments and if the MPI wrappers loads a shared
...
function (_MPI_interrogate_compiler LANG)
  ...
  # Define the MPICH and Intel® MPI Library compiler variables to the compilers set in CMake.
  ...
  # Intel® MPI Library in particular does this with I_MPI_CHECK_COMPILER.
  file(TO_NATIVE_PATH "${CMAKE_${LANG}_COMPILER}" _MPI_UNDERLAYING_COMPILER)
  # On Windows, the Intel® MPI Library batch scripts can only work with filnames - Full paths will break them.
  ...
  # Set these two variables for Intel® MPI Library:
  ...
  #   - I_MPI_FORT_BIND: By default Intel® MPI Library makes the C/C++ compiler wrappers link Fortran bindings.
  ...
  # MPICH, MVAPICH2 and Intel® MPI Library just use "-show". Open MPI also offers this, but the
  ...
  # Unknown to MPICH, MVAPICH and Intel® MPI Library.
  if (NOT MPI_COMPILER_RETURN EQUAL 0)
  ...
  if (MPI_COMPILER_RETURN EQUAL 0 AND DEFINED MPI_COMPILE_CMDLINE)
    # Intel® MPI Library can be run with -compchk or I_MPI_CHECK_COMPILER set to 1.
    ...
    if("${MPI_COMPILE_CMDLINE}" MATCHES "^([^\" ]+/compchk.sh|\"[^\"]+/compchk.sh\") +([^ ]+)")
      ...
      if(NOT MPI_COMPILER_RETURN EQUAL 0)
        if(NOT MPI_FIND_QUIETLY)
          message(STATUS "Intel® MPI Library compiler check failed: ${COMPILER_CHECKER_OUTPUT}")
        endif()
      else()
  ...
  if(MSVC)
    foreach(_MPI_VARIABLE IN ITEMS COMPILE LINK)
      # The Intel® MPI Library wrappers on Windows prefix their output with some copyright boilerplate.
  ...
  if(UNIX)
    ...
    # If Intel® MPI Library was configured for static linkage with -static_mpi, the wrapper will by default strip
...
function(_MPI_adjust_compile_definitions LANG)
  if("${LANG}" STREQUAL "CXX")
    ...
    if(MPI_CXX_SKIP_MPICXX AND NOT MPI_${LANG}_COMPILE_DEFINITIONS MATCHES "SKIP_MPICXX")
      # MPICH_SKIP_MPICXX is being used in MPICH and derivatives like MVAPICH or Intel® MPI Library
      ...
      if(NOT "${MPI_${LANG}_COMPILER}" STREQUAL "${CMAKE_${LANG}_COMPILER}" OR NOT MPI_${LANG}_WORKS)
        ...
        if(NOT MPI_SKIP_COMPILER_WRAPPER)
          ...
          if("${MPI_${LANG}_COMPILER}" STREQUAL "${CMAKE_${LANG}_COMPILER}")
            ...
            # Otherwise, MPI compilers for which CMake has separate linking stages, e.g. Intel® MPI Library on Windows where link.exe is being used
