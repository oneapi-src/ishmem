...
#include "pmi-simple/pmi-simple_config.h"
...
{
    if (maxlen == nullptr) return PMI_ERR_INVALID_ARG;
...
int PMI_KVS_Put(const char kvsname[], const char key[], const char value[])
{
    ...
    if (PMI_initialized == SINGLETON_INIT_BUT_NO_PM) {
        if (cached_singinit_inuse) return PMI_FAIL;
...
int PMI_Publish_name(const char service_name[], const char port[])
{
    ...
    if (PMI_initialized > SINGLETON_INIT_BUT_NO_PM) {
        MPL_snprintf(cmd, PMIU_MAXLINE, "cmd=publish_name service=%s port=%s\n", service_name,
                     port);
...
int PMI_Spawn_multiple(int count, const char *cmds[], const char **argvs[], const int maxprocs[],
                       const int info_keyval_sizes[], const PMI_keyval_t *info_keyval_vectors[],
                       int preput_keyval_size, const PMI_keyval_t preput_keyval_vector[],
                       int errors[])
{
    ...
    for (spawncnt = 0; spawncnt < count; spawncnt++) {
        ...
        rc = MPL_snprintf(buf, PMIU_MAXLINE, "mcmd=spawn\nnprocs=%d\nexecname=%s\n",
                          maxprocs[spawncnt], cmds[spawncnt]);
        if (rc < 0) {
            return PMI_FAIL;
        }

        rc = MPL_snprintf(tempbuf, PMIU_MAXLINE, "totspawns=%d\nspawnssofar=%d\n", count,
                          spawncnt + 1);
        ...
        argcnt = 0;
        if ((argvs != nullptr) && (argvs[spawncnt] != nullptr)) {
            for (i = 0; argvs[spawncnt][i] != nullptr; i++) {
...
static int PMII_getmaxes(int *kvsname_max, int *keylen_max, int *vallen_max)
{
    ...
    rc = MPL_snprintf(buf, PMIU_MAXLINE, "cmd=init pmi_version=%d pmi_subversion=%d\n", PMI_VERSION,
                      PMI_SUBVERSION);
    if (rc < 0) {
        return PMI_FAIL;
    }
    ...
    if (strncmp(cmd, "response_to_init", PMIU_MAXLINE) != 0) {
        MPL_snprintf(errmsg, PMIU_MAXLINE, "got unexpected response to init :%s: (full line = %s)",
                     cmd, buf);
        PMI_Abort(-1, errmsg);
    } else {
        ...
        if (strncmp(buf, "0", PMIU_MAXLINE) != 0) {
            ...
            MPL_snprintf(errmsg, PMIU_MAXLINE, "pmi_version mismatch; client=%d.%d mgr=%s.%s",
