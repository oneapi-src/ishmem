...
int main(int argc, char **argv)
{
    ...
    /* Add up the results from all PEs */
    if (ishmem_size_sum_reduce(inside, inside, 1)) {
        std::cerr << "ishmem reduce failed, exiting" << std::endl;
        ishmem_free(inside);
        return EXIT_FAILURE;
    }

    /* Copy to host */
    size_t *inside_h = (size_t *) sycl::malloc_host<size_t>(1, q);
    q.memcpy(inside_h, inside, sizeof(size_t)).wait();
